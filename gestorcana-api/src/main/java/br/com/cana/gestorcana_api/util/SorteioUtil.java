...
public class SorteioUtil {
    ...
    public static void realizarSorteio(Partida partida, List<Jogador> presentes, String formacao) {
        ...
        for (String pos : posicoesLinha) {
            ...
            candidatos.sort(Comparator.comparingInt((Jogador j) -> j.getNivel() != null ? j.getNivel() : 0).reversed()); // Equilibra pelo nível técnico
        ...
        // Se sobrou algum goleiro reserva, manda pra sobra também
        ...
        sobras.sort(Comparator.comparingInt((Jogador j) -> j.getNivel() != null ? j.getNivel() : 0).reversed());
    ...
    public static int calcularSomaNivel(List<Jogador> time) {
        return time.stream().mapToInt(j -> j.getNivel() != null ? j.getNivel() : 0).sum();
    }
}