...
public class FormJogadorView extends JFrame {
    ...
    private JPanel criarPainelEsportivas() {
        ...
        cbPosicao = new JComboBox<>(new String[] { "Goleiro", "Zagueiro", "Lateral", "Volante", "Meia", "Atacante" });
    ...
    private void acaoSalvar() {
        ...
        Object itemPadrinho = cbPadrinho.getSelectedItem();
        if (itemPadrinho instanceof Jogador) {
            Jogador padrinho = (Jogador) itemPadrinho;

            // 🔒 PROTEÇÃO: Só valida se o jogadorAtual já tiver um ID salvo (Edição)
            if (jogadorAtual.getId() != null && jogadorAtual.getId() > 0) {
                if (padrinho.getId() != null && padrinho.getId().equals(jogadorAtual.getId())) {
        ...
        // Se for um novo jogador (sem ID), zera o ID do endereço para evitar conflito na API
        if (jogadorAtual.getId() == null || jogadorAtual.getId() == 0) {
            if (jogadorAtual.getEndereco() != null) {
                jogadorAtual.getEndereco().setId(null);
            }
        }

        System.out.println("----- DEBUG DO SALVAR -----");
        System.out.println("O que o Java leu da tela: " + jogadorAtual.getNumCamisa());
        System.out.println("ID do Endereço indo pro banco: "
                + (jogadorAtual.getEndereco() != null ? jogadorAtual.getEndereco().getId() : "NULO (Correto)"));
        System.out.println("---------------------------");

        try {
            // 🌟 LER AS DATAS DENTRO DO TRY-CATCH
            System.out.println(">>> Lendo data de nascimento...");
            jogadorAtual.setDataNasc(br.com.cana.util.DateUtil.ler(txtDataNasc.getText()));

            System.out.println(">>> Lendo data de admissão...");
            jogadorAtual.setDataAdmissao(br.com.cana.util.DateUtil.ler(txtDataAdmissao.getText()));

            System.out.println(">>> Gerando JSON payload...");
            String payload = ApiClient.GSON.toJson(jogadorAtual);
            System.out.println(">>> Payload gerado: " + payload);

            String resultado;
            if (jogadorAtual.getId() != null && jogadorAtual.getId() > 0) {
                System.out.println(">>> Enviando PUT para a API...");
                resultado = ApiClient.put("/jogadores/" + jogadorAtual.getId(), payload);
            } else {
                System.out.println(">>> Enviando POST para a API...");
                resultado = ApiClient.post("/jogadores", payload);
            }
            ...
            if (resultado != null) {
                ...
                JOptionPane.showMessageDialog(this, "Erro ao salvar jogador (API retornou nulo).", "Aviso",
                        JOptionPane.WARNING_MESSAGE);
            }

        } catch (Exception ex) {
            ex.printStackTrace(); // 👈 Imprime o erro detalhado no terminal
            JOptionPane.showMessageDialog(this, "Erro ao processar/salvar jogador: " + ex.getMessage(), "Erro",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void acaoExcluir() {
        ...
        if (confirma == JOptionPane.YES_OPTION) {
            try {
                // Chamada DELETE para a API passando o ID do jogador
                ApiClient.delete("/jogadores/" + jogadorAtual.getId());
    ...
    private void popularObjeto() {
        ...
        jogadorAtual.setDataNasc(DateUtil.ler(txtDataNasc.getText()));
        ...
        // Agora sim, pega os valores (se o usuário digitou algo inválido, ele vai usar
        // o último valor válido, evitando travar a tela)
        jogadorAtual.setAltura(Double.parseDouble(spAltura.getValue().toString()));
        ...
        if (padrinhoSelecionado != null && padrinhoSelecionado.getId() != null && padrinhoSelecionado.getId() > 0) {
    ...
    private void preencherCampos() {
        ...
        txtDataNasc.setText(br.com.cana.util.DateUtil.paraUsuario(jogadorAtual.getDataNasc()));
        ...
        if (jogadorAtual.getPadrinhoId() != null && jogadorAtual.getPadrinhoId() > 0) {
            for (int i = 0; i < cbPadrinho.getItemCount(); i++) {
                ...
                // Proteção contra ID nulo usando Objects.equals ou Integer.equals
                if (p != null && p.getId() != null && p.getId().equals(jogadorAtual.getPadrinhoId())) {
    ...
    private void carregarPadrinhos() {
        ...
        // Busca todos os jogadores via API
        try {
            String json = ApiClient.get("/jogadores");

            java.lang.reflect.Type type = new com.google.gson.reflect.TypeToken<java.util.List<Jogador>>() {
            }.getType();

            java.util.List<Jogador> lista = ApiClient.GSON.fromJson(json, type);

            if (lista != null) {
                // 🎯 ORDENAÇÃO DE A a Z: Trata valores nulos no nome para não estourar exceção
                lista.sort((j1, j2) -> {
                    if (j1.getNome() == null)
                        return -1;
                    if (j2.getNome() == null)
                ...
                for (Jogador j : lista) {
                    // Regra tratada contra null:
                    // Se jogadorAtual.getId() for null (cadastro novo), insere todos.
                    // Se for edição, só insere se o ID do padrinho for diferente do ID do jogador
                    // atual.
                    boolean eMesmoJogador = jogadorAtual.getId() != null
                            && j.getId() != null
                            && jogadorAtual.getId().equals(j.getId());
            ...
            JOptionPane.showMessageDialog(this, "Erro ao carregar lista de padrinhos da API: " + ex.getMessage(),
    ...
    private void buscarCepNaInternet() {
        ...
        if (cep.length() == 8) {
            // Executa em uma Thread separada para a tela não dar uma "travadinha" enquanto
            // a internet carrega
            new Thread(() -> {
                ...
                if (!dados.isEmpty()) {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        ...
                        // Joga o cursor automaticamente para o campo de "Número" para o usuário
                        // continuar digitando
                        txtNum.requestFocus();
                    });
                }
