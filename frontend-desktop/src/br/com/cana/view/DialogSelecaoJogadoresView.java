...
public class DialogSelecaoJogadoresView extends JDialog {
    ...
    public DialogSelecaoJogadoresView(Frame parent, List<Jogador> jogadoresAtivos, List<Integer> idsDevedores) {
        this(parent, jogadoresAtivos, idsDevedores, null);
    }

    // preSelecionados: seleção confirmada da última vez que esta tela foi aberta
    // durante a mesma sessão de criação de partida, para não zerar ao reabrir.
    public DialogSelecaoJogadoresView(Frame parent, List<Jogador> jogadoresAtivos, List<Integer> idsDevedores,
            List<String> preSelecionados) {
        super(parent, "Selecionar Jogadores Presentes", true);
        if (preSelecionados != null) {
            jogadoresNaOrdemDeChegada.addAll(preSelecionados);
        }
        ...
        if (jogadoresAtivos != null) {
            for (Jogador j : jogadoresAtivos) {
                ...
                String nomeBase = (j.getApelido() != null && !j.getApelido().trim().isEmpty())
                        ? j.getApelido()
                        : (j.getNome() != null ? j.getNome() : "Jogador #" + j.getId());

                String nomeExibir = nomeBase;
                ...
                final String apelidoPuro = nomeBase;

                if (jogadoresNaOrdemDeChegada.contains(apelidoPuro)) {
                    cb.setSelected(true);
                }
                    ...
                    if (cb.isSelected()) {
                        if (isDevedor) {
                            int opcao = JOptionPane.showConfirmDialog(DialogSelecaoJogadoresView.this,
                                    "O jogador " + apelidoPuro
                                            + " possui mensalidades atrasadas.\nDeseja escalá-lo mesmo assim?",
        ...
        txtBusca.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void filtrar() {
                ...
                for (JCheckBox cb : checkBoxes) {
                    boolean visivel = termo.isEmpty() || (cb.getText() != null && cb.getText().toLowerCase().contains(termo));
                    cb.setVisible(visivel);
                }
